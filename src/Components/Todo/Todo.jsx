...
export const Todo = () => {
  ...
  const handleTodoCheckChange =(todoId) => {
    const updatedTodoList = todoList.map(todo => todoId === todoId ? {...todo, isCompleted: !todo.isCompleted}: todo);
    setTodoList(updatedTodoList);
            ...
            return (
              <div key={_id} className="todo-items d-flex align-center">
                <label className={`${isCompleted ? "strike-through" : ""} todo-label`}>
                  ...
                  <input className="todo-check" type="checkbox" onChange={() => handleTodoCheckChange(_id)} checked={isCompleted}/> {todo}
                </label>
